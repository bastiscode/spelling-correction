...
class SpellingCorrectionServer(TextCorrectionServer):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        def _evaluate() -> Response:
            ...
            try:
                ...
                (f1_micro, *_), _ = metrics.spelling_correction_f1(
                    json["input"],
                    json["output"],
                    json["groundtruth"],
                    sequence_averaged=False
                )
                (f1_seq_avg, *_), infos = metrics.spelling_correction_f1(
                    json["input"],
                    json["output"],
                    json["groundtruth"],
            ...
            return jsonify({
                "metrics": [
                    {"name": "Sequence accuracy", "score": seq_acc *
                        100, "largerIsBetter": True, "precision": 2},
                    {"name": "Micro-averaged F1", "score": f1_micro *
                        100, "largerIsBetter": True, "precision": 2},
                    {"name": "Sequence-averaged F1", "score": f1_seq_avg *
                        100, "largerIsBetter": True, "precision": 2},
                ],
                "info": [
                    {"tp": tp, "fp": fp, "fn": fn}
