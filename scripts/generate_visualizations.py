...
from spelling_correction.api import SpellingCorrector
...
def parse_args() -> argparse.Namespace:
    ...
    model_group.add_argument(
        "-m",
        "--model",
        choices=[model.name for model in SpellingCorrector.available_models()],
        default=SpellingCorrector.default_model().name,
        help=f"Name of the model to use for {SpellingCorrector.task}"
    )
    model_group.add_argument(
        "-e",
        "--experiment",
...
def generate_visualizations(args: argparse.Namespace) -> None:
    ...
    if args.experiment is not None:
        cor = SpellingCorrector.from_experiment(
            args.experiment, device="cpu")
    else:
        cor = SpellingCorrector.from_pretrained(args.model, device="cpu")
    ...
    for layer_name, attention_weights in zip(layer_names, all_weights):
        ...
        if args.type == "attention":
            ...
            for head in range(attention_weights.shape[0]):
                logger.info(
                    f"generating attention chart for layer {layer_name} and head {head + 1}")
                chart = attention_chart(
